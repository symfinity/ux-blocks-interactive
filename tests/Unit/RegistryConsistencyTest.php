...
final class RegistryConsistencyTest extends TestCase
{
    ...
    public static function interactiveRoleProvider(): array
    {
        $provider = [];
        foreach (InteractiveRoleCatalog::roles() as $role) {
            $provider[$role] = [$role];
        }
    ...
    public function yamlSchemaVersionMatchesContract(): void
    {
        ...
        self::assertSame('blocks.int', $registry['registry_prefix']);
    }

    #[Test]
    public function yamlContainsAllInteractiveRoles(): void
    {
        $registry = $this->loadRegistry();
        $roles = array_column($registry['roles'], 'role');

        self::assertSame(InteractiveRoleCatalog::roles(), $roles);
    }

    #[Test]
    #[DataProvider('interactiveRoleProvider')]
    public function eachRowHasRequiredFields(string $role): void
    {
        ...
        self::assertSame('blocks.int.' . $role, $row['fragment_id']);
        self::assertSame('blocks.int.' . $role . '.{n}', $row['fragment_pattern']);
