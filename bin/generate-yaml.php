...
foreach ($liveRoles as $r) {
    ...
    if ($r['role'] === 'data-table-chrome-interactive') {
        $liveAliases['blocks.ext.data-table-chrome'] = 'blocks.int.data-table-chrome-interactive';
        continue;
    }
    $liveAliases['blocks.ext.' . $r['role']] = 'blocks.int.' . $r['role'];
}
...
dumpYaml($root . '/ux-blocks-interactive/config/ux_roles.yaml', '1.3', 'blocks.int', 'UxBlocksLive', $liveRoles, 'stl', $liveAliases);
